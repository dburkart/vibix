...
cfg_if! {
    if #[cfg(target_os = "aix")] {
        ...
        mod l4re;
        // pub(crate) use l4re::*;
    } else if #[cfg(any(target_os = "linux", target_os = "vibix"))] {
        mod linux_uapi;
...
cfg_if! {
    ...
    if #[cfg(all(
        any(target_family = "unix", target_os = "vibix"),
        any(target_env = "gnu", target_os = "vibix"),
...
cfg_if! {
    if #[cfg(all(any(target_family = "unix", target_os = "vibix"), not(target_os = "qurt")))] {
