...
fn open_with_flags(path: &[u8], flags: u32) -> i64 {
    let uva = stage(path);
    unsafe { syscall_dispatch(core::ptr::null_mut(), SYS_OPEN, uva, flags as u64, 0, 0, 0, 0) }
}

fn open_rw(path: &[u8], flags: u32, mode: u32) -> i64 {
    let uva = stage(path);
    unsafe { syscall_dispatch(core::ptr::null_mut(), SYS_OPEN, uva, flags as u64, mode as u64, 0, 0, 0) }
}
...
fn pipe2(flags: u32) -> (i32, i32) {
    ...
    let r = unsafe { syscall_dispatch(core::ptr::null_mut(), SYS_PIPE2, fds_va, flags as u64, 0, 0, 0, 0) };
...
fn fcntl_setfl_o_nonblock_on_pipe_read_returns_eagain() {
    ...
    install_user_staging_vma();
    let buf_va = USER_PAGE_VA as u64 + 2 * 4096;
    let n = unsafe { syscall_dispatch(core::ptr::null_mut(), SYS_READ, rfd as u64, buf_va, 8, 0, 0, 0) };
    assert_eq!(
        n, EAGAIN,
