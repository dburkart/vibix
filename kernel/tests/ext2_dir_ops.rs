...
fn make_ext2_inode_from_stat(
    ...
    let inodes_per_group = super_arc.sb_disk.s_inodes_per_group;
    let group = (ino - 1) / inodes_per_group;
    let index_in_group = (ino - 1) % inodes_per_group;
    let bg = &super_arc.bgdt[group as usize];
    ...
    let absolute_block = bg.bg_inode_table as u64 + block_in_table;
