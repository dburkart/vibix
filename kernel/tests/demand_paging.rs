//! Integration test: an `AnonObject`-backed VMA region is resolved
//! lazily by the `#PF` handler. Proves the demand-paging path — first
//! read faults, lands zero, write-then-read preserves the sentinel
...
fn install_test_vma() {
    ...
    vibix::task::install_vma_on_current(Vma::new(
        ...
        prot_pte,
        Share::Private,
        AnonObject::new(Some(TEST_LEN / 4096)),
        0,
    ));
}
...
fn anon_zero_first_touch() {
    install_test_vma();
    // First touch faults; handler resolves via AnonObject::fault and
    // map_existing_in_pml4, then the read returns zero.
    let byte = unsafe { ptr::read_volatile(TEST_VA as *const u8) };
    assert_eq!(byte, 0, "fresh AnonObject page did not read as zero");
}
...
fn anon_zero_readback() {
    ...
    assert_eq!(first, 0, "second AnonObject page did not read as zero");
