...
pub fn exec_atomic(elf_bytes: &'static [u8]) -> Result<core::convert::Infallible, i64> {
    ...
    let new_fs_base = image.tcb_addr.unwrap_or(0);
    crate::task::set_current_fs_base(new_fs_base);
    // Write the hardware MSR immediately — jump_to_ring3 enters user mode
    // without a context switch, so the switch-in restore path won't run.
    unsafe { Msr::new(MSR_FS_BASE).write(new_fs_base) };
