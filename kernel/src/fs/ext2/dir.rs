...
mod block_backed {
    ...
    use super::super::file::build_metadata_map;
    use super::super::indirect::{resolve_block, Geometry, WalkError};
    ...
    pub fn lookup(super_ref: &Arc<Ext2Super>, dir: &Ext2Inode, name: &[u8]) -> Result<u32, i64> {
        ...
        for logical in 0..block_count {
            ...
            let abs = match resolve_block(
                ...
                Ok(Some(a)) => a,
                // A hole within the directory's reported size is
                // spec-illegal (ext2 directories are never sparse); the
                // walker surfaces it as `Ok(None)`. Since `logical` is
                // strictly `< block_count = size.div_ceil(block_size)`,
                // an in-range hole is on-disk corruption — treat it as
                // EIO rather than silently skipping records.
    ...
    pub fn getdents64(
        ...
        for logical in 0..block_count {
            ...
            let abs = match resolve_block(
                ...
                Ok(Some(a)) => a,
                // As above: an in-range hole in a directory is
                // corruption, not a skip.
                Ok(None) => return Err(EIO),
                Err(WalkError::Io) => return Err(EIO),
                Err(WalkError::Corrupt) => return Err(EIO),
            };
